...
The methodology used for calculating the intrinsic value per share for a stock in a nutshell: Total net operating cash the company is expected to generate over a period of ten years divided by the number of shares outstanding, plus cash per share, minus debt per share, plus total expected dividend over 10 years. That intrinsic value score is then combined with the Wall St. consensus target price and an "Average" price is output along with the other columns of data. The best stocks are those with an Intrinsic Value and Target Price above the current trading price. 
